...
return {
  ...
  config = function()
    ...
    -- n モードの t/T は telescope/noice に割当済み (keymaps/init.lua) のため x/o のみ
    vim.keymap.set({ 'x', 'o' }, 't', function() ft({ offset = -1, opts = clever_t }) end)
    vim.keymap.set({ 'x', 'o' }, 'T', function() ft({ backward = true, offset = 1, opts = clever_t }) end)
  end,
}
